...
@Component({
  selector: 'app-retailer-upload',
  imports: [CommonModule, RouterModule, FormsModule, NgSelectModule],
  templateUrl: './retailer-upload.component.html',
  styleUrl: './retailer-upload.component.css',
})
export class RetailerUploadComponent {
  //! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ROUTE CHECK <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  get isAddHeadquartersRoute(): boolean {
    return this.router.url.includes('/upload/retailer-upload');
  }
  ...
  closeCard(): void {
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/upload/retailer-upload']);
    });
  }
