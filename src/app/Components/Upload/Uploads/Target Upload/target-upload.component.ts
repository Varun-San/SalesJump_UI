...
@Component({
  selector: 'app-target-upload',
  imports: [CommonModule, RouterModule, FormsModule, NgSelectModule],
  templateUrl: './target-upload.component.html',
  styleUrl: './target-upload.component.css',
})
export class TargetUploadComponent {
  //! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ROUTE CHECK <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  get isAddHeadquartersRoute(): boolean {
    return this.router.url.includes('/upload/target-upload');
  }
  ...
  closeCard(): void {
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/upload/target-upload']);
    });
  }
