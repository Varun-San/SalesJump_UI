...
@Component({
  ...
  styleUrl: './pending-bills-upload.component.css',
})
export class PendingBillsUploadComponent {
  //! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ROUTE CHECK <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  get isAddHeadquartersRoute(): boolean {
    return this.router.url.includes('/upload/pending-bills-upload');
  }
  ...
  closeCard(): void {
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/upload/pending-bills-upload']);
    });
  }
  ...
  downloadFile() {
    ...
    link.href = 'Assets/Sample-files/File_example_csv_5_date_format.csv';
    link.download = 'File_example_csv_5_date_format.csv';
