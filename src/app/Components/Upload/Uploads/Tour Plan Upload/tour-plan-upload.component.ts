...
@Component({
  selector: 'app-tour-plan-upload',
  imports: [CommonModule, RouterModule, FormsModule, NgSelectModule],
  templateUrl: './tour-plan-upload.component.html',
  styleUrl: './tour-plan-upload.component.css',
})
export class TourPlanUploadComponent {
  //! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ROUTE CHECK <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  get isAddHeadquartersRoute(): boolean {
    return this.router.url.includes('/upload/tour-plan-upload');
  }
  ...
  closeCard(): void {
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/upload/tour-plan-upload']);
    });
  }
