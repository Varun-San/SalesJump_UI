...
@Component({
  selector: 'app-primary-target-upload',
  imports: [CommonModule, RouterModule, FormsModule, NgSelectModule],
  templateUrl: './primary-target-upload.component.html',
  styleUrl: './primary-target-upload.component.css',
})
export class PrimaryTargetUploadComponent {
  //! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ROUTE CHECK <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  get isAddHeadquartersRoute(): boolean {
    return this.router.url.includes('/upload/primary-target-upload');
  }
  ...
  closeCard(): void {
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/upload/primary-target-upload']);
    });
  }
