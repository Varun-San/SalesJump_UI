...
@Component({
  ...
  styleUrl: './bulk-user-upload.component.css',
})
export class BulkUserUploadComponent {
  //! >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ROUTE CHECK <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  get isAddHeadquartersRoute(): boolean {
    return this.router.url.includes('/upload/bulk-user-upload');
  }
  ...
  closeCard(): void {
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/upload/bulk-user-upload']);
    });
  }
  ...
  downloadFile() {
    ...
    link.href = 'Assets/Sample-files/Employee_Upload_Format.xlsx';
    link.download = 'Employee_Upload_Format.xlsx';
